...
class Parser < Parslet::Parser
  ...
  rule(:unichar16) { str('\u') >> xdigit.repeat(4, 4) }
  rule(:unichar32) { str('\U') >> xdigit.repeat(8, 8) }
  ...
  rule(:identifier) {
    (alpha >> (alpha | unichar16 | unichar32 | digit).repeat).as(:identifier) >> spaces?
  }
  ...
  rule(:declaration_specifiers) {
    (
      ...
      function_specifier.as(:specifier) |
  ...
  rule(:enum_specifier) {
    enum_keyword >> (
      (
        identifier.maybe >> (left_brace >> enumerator_list >> comma.maybe >> right_brace)
      ) | identifier
    )
  }
  ...
  rule(:type_qualifier) { const_keyword | restrict_keyword | volatile_keyword }
